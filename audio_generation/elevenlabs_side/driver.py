"""Driver for the ElevenLabs TTS engine. Made by AI.
TODO: review this
"""
import json
import os
...
def _load_env_file(env: dict, dotenv_path: Path, *, override: bool = False) -> None:
    """Minimal .env loader: KEY=VALUE lines, supports optional `export` and quoted values."""
    try:
        ...
        # do not fail the run if .env parsing fails; rely on existing env
        pass
...
def run_internals_synthesizer(
    ...
    *,
    ...
    """Launch the ElevenLabs internals synthesizer using the internals venv if available.
    ...
    # Load internals/.env if present so ELEVENLABS_API_KEY is available
    _load_env_file(env, _internals_dir() / ".env", override=False)
    # Requires ELEVENLABS_API_KEY in environment
    env["SYNTH_ARGS"] = json.dumps(args_payload)
    with tempfile.NamedTemporaryFile(prefix="eleven_records_", suffix=".json", delete=False) as tf:
